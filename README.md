...
> **Unofficial community project.** A source-first Windows launcher that keeps the normal Codex App profile as the primary profile and starts an optional, locally isolated work profile. Requires Windows, the Codex Windows App, and .NET 10 SDK to build. It is not affiliated with or endorsed by OpenAI or Microsoft. The isolated launch path depends on an undocumented compatibility surface and locally copies installed App files; read the [compatibility and terms notice](docs/COMPATIBILITY.md) before use. No binaries are distributed in this preview.

Codex 多开器是一个 Windows 本机 GUI，用于明确选择：

- **个人空间**：通过原始 Store 注册入口启动，继续使用默认登录和 `%USERPROFILE%\.codex`；
- **工作空间**：使用独立 Codex Home、Electron 用户数据和本机运行副本，可连接用户自行配置的 OpenAI Responses API 兼容 Provider。

工作空间是可选的，个人空间始终是默认主入口。启动器不会为了切换通道而覆盖共同的全局 `config.toml` 或 `auth.json`。
...
- 工作空间 Provider 必须支持 OpenAI Responses API 线协议。
...
首次打开多开器时会显示工作空间向导。可以选择：

### 新建工作空间

填写显示名、Provider ID、Provider 名称、Base URL、模型、推理等级和 API Key。
...
如果注册文件不存在，但 `profiles/*/codex-home` 中只发现一个带有效旧 marker 的 profile，多开器会原地登记并继续使用：不移动、不复制、不重写其 `config.toml` 或 `auth.json`。若发现多个候选，向导会要求明确选择。
